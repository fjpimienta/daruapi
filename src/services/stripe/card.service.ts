...
class StripeCardService extends StripeApi {
    ...
    return await this.execute(
      ...
      return {
        ...
        message: result.deleted ?
          `El elemento ${result.id} ha sido borrado satisfactoriamente` :
          `El elemento ${result.id} no se ha borrado`,
        id: result.id
      };
    }).catch((error: Error) => this.getError(error));
  }
  ...
  async list(customer: string, limit: number = 5, startingAfter: string = '', endingBefore: string = '') {
    ...
    return await this.execute(
      ...
      return {
        status: true,
        message: `Lista de tarjetas mostrados correctamente`,
  ...
  // Eliminar las tarjetas que no estan siendo usadas
