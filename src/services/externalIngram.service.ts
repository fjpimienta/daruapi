    ...
    try {
      ...
      if (item.availability && item.availability.availabilityByWarehouse && item.availability.availabilityByWarehouse.length > 0) {
        ...
        if (branchOfficesIngram.length > 0) {
          ...
          itemData.especificaciones.push({ agrupadoPor: 'Dimensiones', tipo: 'Longitud', valor: productJson.length });
          itemData.especificaciones.push({ agrupadoPor: 'Dimensiones', tipo: 'Ancho', valor: productJson.width });
          itemData.especificaciones.push({ agrupadoPor: 'Dimensiones', tipo: 'Altura', valor: productJson.height });
          itemData.sheetJson = productJson.sheetJson;
        }
      }
      return itemData;
    } catch (error) {
